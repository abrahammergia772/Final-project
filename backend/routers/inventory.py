...
def _prophet_scale(drug: str, d: date) -> float:
    ...
    try:
        ...
        yearly = data.get("yearly", data.get("yearly_seasonality", {}))
        w = 1.0
...
def forecast_inventory(req: ForecastRequest):
    ...
    # normalize: "Paracetamol 500mg" -> "Paracetamol"
    base = drug.split(" ")[0].title()
    cat = cfg.get("drug_catalogue", [])
    drug_key = base if base in cat else (cat[0] if cat else "Paracetamol")

    days = max(1, min(int(req.days or 30), 180))
    ...
    rng = np.random.default_rng(abs(hash(drug_key)) % 2 ** 32)
