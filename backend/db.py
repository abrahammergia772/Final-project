...
def list_rows(endpoint: str, limit: int = 500) -> Dict[str, Any]:
    ...
    items = [_present(endpoint, _hide_secrets(dict(item))) for item in DEMO.get(endpoint, [])]
    return {"ok": True, "items": items, "total": len(items), "source": "demo"}
...
def insert_row(endpoint: str, data: dict) -> Dict[str, Any]:
    ...
    return {"ok": True, "row": _hide_secrets(dict(row)), "source": "demo"}
...
def update_row(endpoint: str, row_id: str, data: dict) -> Dict[str, Any]:
    ...
    for r in rows:
        if r.get("id") == row_id:
            r.update(payload)
            return {"ok": True, "row": _hide_secrets(dict(r)), "source": "demo"}
