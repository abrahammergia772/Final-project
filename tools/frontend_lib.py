# -*- coding: utf-8 -*-
"""MedIQ Pro frontend generator — shared library.
Icons, HTML helpers and the standard page shell. No frameworks, pure static HTML."""
import os
import urllib.parse

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
FRONT = os.path.join(ROOT, "Final-project", "mediq-pro-frontend")

# ---------------------------------------------------------------------------
# Inline SVG icons (Lucide-style, stroke 2, round caps)
# ---------------------------------------------------------------------------
ICON_PATHS = {
...
FAVICON = ("data:image/svg+xml," + urllib.parse.quote(
    ...
    '<path d="M14 46c6 4 12 4 18 0 6 4 12 4 18 0" stroke="#BFDBFE" stroke-width="3" fill="none" stroke-linecap="round"/></svg>'))

FAVICON_ROSE = ("data:image/svg+xml," + urllib.parse.quote(
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">'
    '<rect width="64" height="64" rx="14" fill="#F9A8D4"/>'
    '<path d="M26 14h12v12h12v12H38v12H26V38H14V26h12z" fill="#fff"/></svg>'))


# ---------------------------------------------------------------------------
# Small HTML helpers
# ---------------------------------------------------------------------------
def stat_card(icon_name, value, label, trend=None, trend_dir="up", color=""):
    trend_html = ""
    if trend:
        arrow = "↗" if trend_dir == "up" else "↘"
        cls = "text-success" if trend_dir == "up" else "text-danger"
        trend_html = '<div class="stat-trend ' + cls + '">' + arrow + " " + trend + "</div>"
    return ('<div class="stat-card ' + color + '">'
            '<div class="stat-icon">' + icon(icon_name) + "</div>"
            '<div><div class="stat-value">' + value + '</div>'
            '<div class="stat-label">' + label + "</div>" + trend_html + "</div></div>")


def panel(title, body, sub=None, actions=None, id_=None):
    head = '<div class="panel-header"><div><h3>' + title + "</h3>" + (sub or "") + "</div>" + (actions or "") + "</div>"
    return '<section class="panel"' + ((' id="' + id_ + '"') if id_ else "") + ">" + head + '<div class="panel-body">' + body + "</div></section>"


def badge(text, kind="neutral"):
    return '<span class="badge badge-' + kind + '"><span class="dot"></span>' + text + "</span>"


def pagination(id_):
    return '<div class="pagination" id="' + id_ + '"></div>'


def empty_row(colspan, msg="No records found."):
    return ('<tr class="empty-row"><td colspan="' + str(colspan) + '" style="text-align:center;color:#6B7280;padding:32px">'
            + msg + "</td></tr>")
