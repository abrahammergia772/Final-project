...
| Operating theatre | `doctor/theatre.html` | OT-1, OT-2, maternity theatre |
| Imaging worklist | `doctor/imaging.html` | X-ray, ultrasound, CT |
...
## 🌐 Multi-language support (English / አማርኛ)

Every page (all 96) now supports **runtime switching between English and Amharic** — the
Amharic script loads automatically (Noto Sans Ethiopic).

- **Language button** — segmented **`EN | አማ`** control in the **topbar-right next to the
  notification bell** on every role page, top-right corner on the login/signup pages.
  Shortcut: **Alt + Shift + L**. Sits beside the dark-mode toggle.
- **Light / Dark mode** — sun/moon toggle (same control group); saved per device,
  `?theme=dark` forces dark, follows the OS setting until you choose. Shortcut: **Alt + Shift + T**.
  Dark palette lives in `global.css` under `html[data-theme="dark"]`, engine in `assets/js/theme.js`.
- Choice is saved in `localStorage` and applies on every page; `?lang=am` forces a visit.
- No page rewrites: `assets/js/i18n.js` (engine) + `assets/js/i18n/am.js` (dictionary)
  translate the DOM at runtime, including content generated by page scripts.
- Extend: add entries to `assets/js/i18n/am.js`, or find untranslated strings with
  `?i18n=debug`. See `assets/js/i18n/README.md`.
