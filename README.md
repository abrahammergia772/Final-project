...
> FastAPI backend and trained `.pkl` ML models. The frontend runs immediately in
> demo mode, while the backend can be connected to Supabase for authenticated,
> role-aware data access and AI inference.
...
└── backend/                ← FastAPI app serving the 7 AI models + Supabase (deploy on Render)
    ├── models/             ← trained .pkl models and JSON configuration (per module)
    ├── main.py, config.py, db.py, model_loader.py, download_models.py
    ├── routers/            ← auth + 7 AI modules + role-aware data CRUD
...
python3 -m http.server 8000 --bind 0.0.0.0      # or: npx serve .
# open http://localhost:8000
# For the live API, run it separately from backend on another port.
```
...
3. In `mediq-pro-frontend/assets/js/config.js` set:
   ...
   DEMO_MODE: false,
   ```
   For a deployment that injects configuration before the static assets load,
   `window.MEDIQ_RUNTIME_CONFIG` may provide the same values. `apiFetch()` has
   a 15-second timeout and automatically handles expired sessions.
4. **Supabase**: configure `SUPABASE_URL`, `SUPABASE_KEY`, `SUPABASE_SERVICE_KEY`, and a strong `SECRET_KEY` in the backend environment. The frontend does not need Supabase credentials. Never commit secrets.
...
- The static pages in `mediq-pro-frontend/` are the deployable output. The root
  `_headers` file adds baseline browser hardening on hosts such as Netlify.
- CI checks Python compilation, backend regression tests, and JavaScript syntax
